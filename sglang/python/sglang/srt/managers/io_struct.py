...
class GenerateReqInput:
    ...
    # Explicit sub-context boundaries for --enable-subcontext-kv-cache: token
    # offsets (into this request's own token ids, post-tokenization) the
    # caller knows are a reusable, semantically independent span (a tool
    # output, a RAG fragment, a system-prompt block, ...), each as
    # {"subcontext_id": str, "start": int, "end": int}. Skipped by automatic
    # content-hash scanning, and ignored entirely when the feature is off.
    subcontext_tags: Optional[List[Dict[str, Any]]] = field(
...
class TokenizedGenerateReqInput(BaseReq, kw_only=True):
    ...
    # See GenerateReqInput.subcontext_tags.
