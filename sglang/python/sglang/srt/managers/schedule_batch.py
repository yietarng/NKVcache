...
class Req(ReqDllmMixin):
    ...
    def __init__(
        ...
        subcontext_tags: Optional[List[Dict[str, Any]]] = None,
    ):
        ...
        # Raw caller-supplied spans; resolved into SubContextTag against
        # origin_input_ids lazily in init_next_round_input (subcontext_matches
        # below), since resolution needs the tokenized length to validate
        # offsets. Empty/unused unless --enable-subcontext-kv-cache.
        self.subcontext_tags = subcontext_tags
        # Populated by init_next_round_input: SubContextMatch list (the
        # unmatched-suffix scan result) and, for each, a RecomputePlan.
        self.subcontext_matches: List = []
        self.subcontext_recompute_plans: List = []
    ...
    def init_next_round_input(
        ...
        if tree_cache is not None:
            ...
            if (
                ...
                and len(self.prefix_indices) < input_len
            ):
                tags = tuple(
                    SubContextTag(
                        subcontext_id=tag["subcontext_id"],
                        start=tag["start"],
                        end=tag["end"],
                    )
                    for tag in (self.subcontext_tags or ())
                )
                matches = subcontext_scan(
                    subcontext_index,
                    token_ids_to_match,
                    explicit_tags=tags,
