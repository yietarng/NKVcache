...
class Memory(msgspec.Struct):
    ...
    # -------------------------------------------------------------------------
    # Sub-context KV cache (NOC): position-independent KV reuse
    # -------------------------------------------------------------------------
    enable_subcontext_kv_cache: A[
        bool,
        Arg(
            help=(
                "Enable position-independent KV reuse for repeated sub-contexts "
                "(tool outputs, RAG fragments, system-prompt blocks) that recur "
                "at a different offset than RadixAttention's prefix match can "
                "discover. A registered sub-context's KV is repositioned via a "
                "RoPE delta rotation instead of recomputed. MVP scope only: "
                "dense, standard-RoPE models without MLA / mrope / SWA / Mamba "
                "and without the CUDA-graph fast path; startup fails fast "
                "otherwise. See "
                "https://docs.sglang.io/docs/advanced_features/subcontext_kv_cache."
            ),
            resolvable=True,
        ),
    ] = False
